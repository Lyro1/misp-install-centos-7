...
echo "You can find those credentials in the pwd.txt file"
echo ""

rm -rRf pwd.txt
touch pwd.txt
echo "Admin (root) DB Password: $DBPASSWORD_ADMIN" >> pwd.txt
echo "User  (misp) DB Password: $DBPASSWORD_MISP" >> pwd.txt
...
yum install gcc git httpd zip redis mariadb mariadb-server python-devel python-pip python-zmq libxslt-devel zlib-devel python36u python36u-libs python36u-devel python36u-pip -y
